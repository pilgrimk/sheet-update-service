...
class SheetWriter:
    def apply_updates(
        ...
        Duplicate target cells are deduplicated using "last value wins".
        A warning is returned whenever later updates overwrite earlier ones
        within the same request payload.
        """
        deduped_updates = []
        latest_by_cell: dict[tuple[int, int], dict] = {}
        ...
        for index, item in enumerate(updates):
            ...
            if cell_key in latest_by_cell:
                previous = latest_by_cell[cell_key]
                duplicate_overwrites.append({
                    "row": row,
                    "col": col,
                    "a1": gspread.utils.rowcol_to_a1(row, col),
                    "overwritten_value": previous.get("value"),
                    "final_value": item.get("value"),
                    "metric": item.get("metric"),
                    "asin": item.get("asin"),
                    "date": item.get("date"),
                    "message": "Duplicate target cell detected in request. Last value wins.",
                })

            latest_by_cell[cell_key] = {
                **item,
                "_last_seen_index": index,
            }

        deduped_updates = sorted(
            latest_by_cell.values(),
            key=lambda item: item["_last_seen_index"]
        )

        warnings = []
        if duplicate_overwrites:
            warnings.append(
                f"{len(duplicate_overwrites)} duplicate cell update(s) detected. Last value wins was applied."
            )
        ...
        if dry_run:
            return {
                ...
                "updates_applied": 0,
                "updates_after_deduplication": len(deduped_updates),
                "mode": "dry_run",
                "applied_cells": [],
                "warnings": warnings,
